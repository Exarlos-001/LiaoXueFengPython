'''
Author: Exarlos
Date: 2022-06-17 01:19:37
LastEditors: Exarlos
LastEditTime: 2022-06-18 22:27:23
...
# 安装好 MySQL 后， 第一步：从开始菜单打开服务-打开
# 第二步：开始菜单打开 'MySQL 5.6 Command Line Client' ，并输入安装时你设置的 root 密码
# 输密码的时候不会显示密码，你只管输，然后回车
# 第三步：把下面注释的代码复制到 SQL 命令行里，然后回车
# 这里初始化了一个名为 awesome 的数据库表
'''
-- schema.sql
drop DATABASE IF EXISTS awesome;
-- 删掉用户账号
drop user if EXISTS 'python_study'@'localhost';

-- 新建表
CREATE DATABASE awesome;
USE awesome;

CREATE user 'python_study'@'localhost' IDENTIFIED BY '123456';
-- 下面是设置密码的方法
ALTER user 'python_study'@'localhost' IDENTIFIED WITH mysql_native_password BY '123456';
-- 给权限
GRANT ALL PRIVILEGES ON awesome.* TO 'python_study'@'localhost';
...
async def test(loop):                      # *** 注意此处的密码填自己设的密码 ***
    await orm.create_pool(loop=loop, user='python_study', password='123456', db='awesome')
    # *** 注意此处的密码填自己设的密码 ***
